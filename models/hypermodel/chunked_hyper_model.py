...
from . import init_utils as iutils
...
class ChunkedHyperNetworkHandler(nn.Module, CLHyperNetInterface):
    ...
    """
    def __init__(self, target_shapes, num_tasks, chunk_dim=2586, no_te_embs=False, 
                 layers=[50, 100], te_dim=8, activation_fn=torch.nn.ReLU(),
                 use_bias=True, no_weights=False, ce_dim=None,
                 init_weights=None, dropout_rate=-1, noise_dim=-1, externalnorms=None,
                 temb_std=-1, verbose=False):
        ...
        # find the largest fan-in shape in target shape 
        main_fan_in = 0
        for shape in target_shapes:
            if len(shape) > 1:
                main_fan_in = max(main_fan_in, shape[1])
        assert main_fan_in > 0
        self._hypernet = HyperNetwork([[chunk_dim]], num_tasks, verbose=False,
            layers=layers, te_dim=te_dim, activation_fn=activation_fn, no_te_embs=no_te_embs,
            use_bias=use_bias, no_weights=no_weights, init_weights=init_weights,
            ce_dim=ce_dim + (noise_dim if noise_dim != -1 else 0),
            main_fan_in = main_fan_in,
        ...
        if verbose:
            ...
            print('The hypernetwork has a total of %d outputs.' % self._num_outputs + 'Split into %d chunks.' % self._num_chunks)

        self._theta_shapes = [[self._num_chunks, ce_dim//2]] + \
    ...
    def forward(self, task_id=None, theta=None, dTheta=None, task_emb=None,
        ...
        # apply sigmoid to task_emb
        scaled_task_emb = torch.pi * torch.sigmoid(task_emb) / 2
        scaled_task_emb = torch.cat([torch.cos(scaled_task_emb), torch.sin(scaled_task_emb)], dim=-1)
        # apply sigmoid to chunk_embs
        scaled_chunk_embs = torch.pi * torch.sigmoid(chunk_embs) / 2
        scaled_chunk_embs = torch.cat([torch.cos(scaled_chunk_embs), torch.sin(scaled_chunk_embs)], dim=-1)

        # get chunked weights from HyperNet
        weights = self._hypernet.forward(task_id=task_id, theta=hnet_theta,
            dTheta=hnet_dTheta, task_emb=scaled_task_emb, ext_inputs=scaled_chunk_embs)
