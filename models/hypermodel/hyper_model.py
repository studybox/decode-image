    ...
    def _gen_layers(self, layers, te_dim, use_bias, no_weights, init_weights,
        ...
        if init_weights is not None:
            ...
            for i in range(0, len(self._theta), 2 if use_bias else 1):
                if use_bias:
                    if i + 1 == len(self._theta)-1 and self._main_fan_in is not None:
                        init_params(self._theta[i], self._theta[i + 1],
                                    main_fan_in=self._main_fan_in)
                    else:
                        init_params(self._theta[i], self._theta[i + 1])
                else:
                    if i == len(self._theta)-1 and self._main_fan_in is not None:
                        init_params(self._theta[i], fan_in=self._main_fan_in)
                    else:
