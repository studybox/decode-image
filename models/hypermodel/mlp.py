...
from .context_mod_layer import ContextModLayer
...
hyper_shapes_distilled` and the current statistics will be returned by the
    ...
    def __init__(self, n_in=1, n_out=1, hidden_layers=[10, 10],
                 ...
                 use_external_norm=False, external_norm_type=None, ret_before_act=False,
                 ...
                 context_mod_gain_offset=False, out_fn=None, verbose=False):
        ...
        self._use_external_norm = use_external_norm
        self._external_norm_type = external_norm_type
        ...
        self._externalnorm_layers = [] if use_external_norm else None
        if use_batch_norm:
            if distill_bn_stats:
        ...
        if use_external_norm:
            for i, n in enumerate(hidden_layers):
                self._externalnorm_layers.append((external_norm_type, n))
        # Compute shapes of linear layers.
        without_norm = not use_external_norm
        linear_shapes = MLP.weight_shapes(n_in=n_in, n_out=n_out,
            hidden_layers=hidden_layers, use_bias=use_bias, ret_before_act=ret_before_act, without_norm=without_norm)
    ...
    def forward(self, x, weights=None, externalnorms=None, distilled_params=None, condition=None):
        ...
        for l in range(len(w_weights)):
            ...
            if l < len(w_weights) - 1:
                ...
                if self._use_batch_norm:
                    ...
                    bn_ind += 1
                # External norm
                if self._use_external_norm and externalnorms is not None:
                    hidden = externalnorms[en_ind](hidden)
                    en_ind += 1
                # Dropout
    ...
    @staticmethod
    def weight_shapes(n_in=1, n_out=1, hidden_layers=[10, 10], use_bias=True, ret_before_act=False, without_norm=False):
