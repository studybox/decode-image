...
class MultiheadAttentionLocal(nn.Module):
    ...
    def __init__(
        ...
        if self.without_weight:
            ...
            self.out_proj_weight = self.out_proj_bias = None
            self._param_shapes = []
            self._param_shapes.append((self.vdim, self.vdim))
            self._param_shapes.append((self.vdim,))
    ...
    def forward(
        ...
        if self.without_weight:
            ...
            self.in_proj_weight, self.in_proj_bias = None, None
            self.out_proj_weight, self.out_proj_bias = weights
        else:
            assert len(weights) == 4
            self.in_proj_weight, self.in_proj_bias, self.out_proj_weight, self.out_proj_bias = weights
        if vdim is None:
        ...
        # set the parameters to none
        self.in_proj_weight = self.in_proj_bias = self.out_proj_weight = self.out_proj_bias = None
